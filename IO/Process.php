...
class Process
{
    ...
    public static function list(): array
    {
        ...
        for ($i = 3; $i < count($lines); $i++) {
            $wdp = new \UT_Php_Core\Collections\Generic\WindowsDataParser($lines[1], $lines[$i], $lines[2]);
        ...
        return (new \UT_Php_Core\Collections\Linq(array_values($mergedByProcess)))
            -> select(function ($x) {
    ...
    /**
     * @param int $pid
     * @return \UT_Php_Core\Collections\Dictionary
     */
    public function pidInfo(int $pid): \UT_Php_Core\Collections\Dictionary
    {
        $exists = (new \UT_Php_Core\Collections\Linq($this -> processes))
        ...
        return new \UT_Php_Core\Collections\Generic\WindowsDataParser($lines[0], $lines[1]);
    }
    ...
    {
        return (new \UT_Php_Core\Collections\Linq($this -> processes))
            -> select(function ($x) {
                return $x['PID'];
            })
            -> toArray();
    ...
    {
        $sum = (new \UT_Php_Core\Collections\Linq($this -> processes))
            -> sum(function ($x) {
    ...
    {
        $selected = (new \UT_Php_Core\Collections\Linq($this -> processes))
            -> firstOrDefault(function ($x) use ($pid) {
                return $x['PID'] === $pid;
            });
        if ($selected == null) {
